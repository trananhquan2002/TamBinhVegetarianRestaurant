    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 bg-white shadow-md fixed h-full">
        <div className="p-6 border-b">
          <h1 className="text-xl font-bold text-green-600 uppercase">Tâm Bình Admin</h1>
        </div>
        <nav className="p-4 space-y-2">
          {navItems.map((item) => (
      ...
      <div className="flex-1 lg:ml-64 w-full">
        <header className="bg-white p-4 shadow-sm flex justify-between items-center sticky top-0 z-30">
          ...
          <span className="font-medium text-gray-500 hidden sm:inline">Hệ thống quản lý Nhà hàng Chay</span>
          {admin ? (
            ...
            <Link to="/admin/login" className="flex items-center gap-2 bg-red-500 text-white px-5 py-2 rounded-full hover:bg-red-600 transition-all font-medium text-sm shadow-lg active:scale-95 cursor-pointer" title="Đăng nhập">
              <FiUser size={18} />
              <span>Đăng nhập</span>
            </Link>
          )}
