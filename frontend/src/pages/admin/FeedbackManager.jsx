...
export default function FeedbackManager() {
  ...
  const fetchFeedbacks = async () => {
    setLoading(true)
    try {
      const res = await fetch(`${API_BASE_URL}/api/feedbacks?date=${filterDate}`)
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
          <div className="flex items-center bg-gray-50 border border-gray-200 px-3 py-2 rounded-2xl flex-1 min-w-50 focus-within:ring-2 ring-green-500/20 transition-all">
            ...
            <input type="date" value={filterDate} className="bg-transparent text-sm font-bold outline-none w-full cursor-pointer" onChange={(e) => setFilterDate(e.target.value)} />
          </div>
          {filterDate && (
            <button onClick={() => setFilterDate('')} className="px-4 py-2 bg-white border border-gray-200 text-gray-500 rounded-2xl text-[10px] font-black hover:bg-gray-50 transition uppercase shrink-0">
              Tất cả
            </button>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl p-12 sm:p-20 text-center border border-dashed border-gray-200 text-gray-400 font-bold">Chưa có góp ý nào phù hợp.</div>
      ) : (
            ...
            <div key={item._id} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-black text-gray-800 text-base sm:text-lg truncate uppercase tracking-tight">{item.name}</h3>
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                    ...
                    <span className="text-[10px] text-gray-300 font-black uppercase tracking-widest bg-gray-50 px-2 py-0.5 rounded">{new Date(item.createdAt).toLocaleDateString('vi-VN')}</span>
                  </div>
                </div>
              </div>
